...
import static de.volzo.tapper.GestureDetector.FSM.FSMSideTap.sideTapTimeout;

/**
 * Created by tassilokarge on 23.11.16.
 */

public class FSMSideTapUnitTest extends FSMUnitTest {

    private FSMSideTap fsm;

    @Before
    public void setupFSM() {
        this.fsm = new FSMSideTap();
    }
    ...
    public void tap() {
        Assert.assertEquals("fresh machine should be in INIT state",
                FSMSideTap.SideTapState.INIT, fsm.getCurrentSideTapState());
    ...
    private void peakToEndByNothing() {
        ...
        Assert.assertEquals("Machine should now be in END state",
                FSMSideTap.SideTapState.END, fsm.getCurrentSideTapState());
    }
    ...
    private void startOrPeakToPeak() {
        ...
        Assert.assertEquals("Machine should now be in Peak state",
                FSMSideTap.SideTapState.PEAK, fsm.getCurrentSideTapState());
    }
    ...
    private void initialOrStartToStart() {
        ...
        Assert.assertEquals("Machine should now be in Start state",
                FSMSideTap.SideTapState.START, fsm.getCurrentSideTapState());
    }
